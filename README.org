...
Play function into SynthPlayer: 
: { } +> \symbol 

Play function into TaskPlayer:
: { } *> \symbol

Function is used as source for SynthPlayer in both cases. 
In =*>=, the SynthPlayer has the same name as the TaskPlayer.  If a different name is to be used, then provide the name as adverb: 
...
\writer@\out1 @>.in2 \reader; // specify output param out1 and input in2
#+END_SRC
